class Event < ApplicationRecord
  ...
  scope :get_completed_events, -> { where("lucky_ticket_number IS NOT NULL AND event_end_time >= :n_days_before", n_days_before: Time.now - 7.days ) }
  ...
  def event_start_and_end_date
    if event_end_time.present? && event_end_time < Time.now
